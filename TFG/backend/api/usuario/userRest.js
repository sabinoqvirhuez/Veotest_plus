...
function listOneUser(req,res){
  ...
  var sql = "SELECT email, name, surname, password FROM  usuarios WHERE Userid ='"+id+"'";
...
function listUsers(req,res){
  'use strict';
  var mycon = db.doConnection();
  var sql = "Select email, name, surname FROM usuarios";
  //console.log("User global search");
  mycon.query(sql,function(err,result){
    if(err){
      console.log (err);
...
function checkLogIn(email, password, conn) {
  const NOUSER = "NON EXISTENT USER";
  var sql;
  sql = "SELECT Userid, email FROM  usuarios WHERE email = ? AND password = ?";
